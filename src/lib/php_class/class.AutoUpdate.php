	...
	public function install($updateFile) {
		...
		while ($file = zip_read($zip)) {				
			...
			if(!file_exists($this->installDir.$filename)){
				...
				$this->log('Try add new file `'.$this->installDir.$filename.'`');
				fclose($updateHandle);
			}
...
//https://site.ru/local/dev/lp_b24_hosting/lp_b24_hosting.php?className=ShefAutoUpdate&functionName=makeUpdate&token=dekqkddeiorsojdoaixuiqcafwnhwudi&projectId=2
		...
		if ($latest !== false) {
			if ($latest > $update->currentVersion) {
				...
				if ($update->update()) {
					$result = array(
						'response' => array('status' => 'ok', 'message' => 'Update_successful'),
						'values' => array($update->latestVersionName)
					);
				}
				else {
					$result = array(
						'response' => array('status' => 'error', 'message' => 'Update_failed'),
						'values' => array()
					);
				}
				...
				$result = array(
					'response' => array('status' => 'ok', 'message' => 'Current_Version_is_up_to_date'),
					'values' => array()
				);
			}
		}else {
			$result = array(
				'response' => array('status' => 'error', 'message' => $update->getLastError()),
				'values' => array()
			);
		}
		return $result;
