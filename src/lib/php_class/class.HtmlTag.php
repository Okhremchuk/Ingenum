...
class HtmlTag{
    ...
    public function toString(){
        ...
        if(!empty($this->tag)){
            ...
            if($this->autoclosed && empty($this->text) && $this->tag !== 'script') $string .= '>' ;//.CHR(13) . CHR(10)
            else $string .= '>' . ($this->textFirst ?  $this->text.$this->contentToString() : $this->contentToString().$this->text ). '</' . $this->tag . '>'; //CHR(13) . .  CHR(10) . CHR(9)
        }
        else{
    ...
    private function contentToString(){
        ...
        if(!is_null($this->content)){
            foreach($this->content as $c){
                $string .= CHR(13) . CHR(10)  . $c->toString();
            }
        }
        return $string;
    }
